    ...
    <main className={`flex h-[100vh] w-full  items-center justify-between mt-[5rem]   ${theme ? 'bg-[#f7f1e3] text-[#2c3e50]' : 'bg-[#2c3e50] text-white' }`}>
      ...
      <div className=" w-[50%] h-[80%] p-5 flex flex-col items-center self-center text-left ">
        ...
        <AnimatedText
          ...
          className={`!text-5xl ${theme ? '!text-[#2c3e50]' : '!text-white'}`}
          />
        <p className={`my-4 text-base font-medium ${theme ? 'bg-[#f7f1e3] text-[#2c3e50]' : 'bg-[#2c3e50] text-white' }`}>
